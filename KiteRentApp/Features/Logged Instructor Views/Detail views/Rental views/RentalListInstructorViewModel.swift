...
@MainActor
final class RentalListInstructorViewModel: ObservableObject {
    ...
    var filteredAndOrderedRentals: [AdminRental] {
        ...
        if let filterDate = selectedDate {
            dateFilteredRentals = base.filter { rental in
                calendar.isDate(rental.startTime, inSameDayAs: filterDate)
            }
        } else {
        ...
        if searchText.isEmpty {
            ...
            textFilteredRentals = dateFilteredRentals.filter {
                $0.instructorName.localizedCaseInsensitiveContains(searchText)
                    || $0.kiteName.localizedCaseInsensitiveContains(searchText)
            }
        }
    ...
    func loadRentals() async {
        ...
        do {
            ...
            let fetched = try await rentalManager.getRentalsForInstructor(instructorId: uid)
